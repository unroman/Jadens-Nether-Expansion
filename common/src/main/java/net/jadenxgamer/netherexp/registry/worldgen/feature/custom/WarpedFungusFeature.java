...
public class WarpedFungusFeature extends Feature<HugeFungusConfiguration> {
    ...
    private static void tryPlaceBeard(BlockPos pos, LevelAccessor level) {
        ...
        if (level.isEmptyBlock(mutableBlockPos) && level.isEmptyBlock(mutableBlockPos.below()) && level.getBlockState(mutableBlockPos.above()).is(JNETags.Blocks.WART_BEARD_FEATURE_VALID)) {
